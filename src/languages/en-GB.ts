	...
	constructor() {
		super("en-GB", {
			...
			COMMAND_PING_DESCRIPTION:
				"Shows my heartbeat and latency with the Discord API.",
			...
			COMMAND_HELP_EMBED_FOOTER: (
				...
				`${
					guild ? `This server's prefix is "${prefix}" | ` : ""
				}Use "${prefix}help <command>" for details about a command`,
			COMMAND_HELP_EMBED_FIELD_USEFUL_LINKS_NAME: "Useful links",
			COMMAND_HELP_EMBED_FIELD_UNKNOWN_CATEGORY_NAME: "Unknown category",
			COMMAND_HELP_EMBED_DETAILS_FOOTER:
				"<> = required, [] = optional, | = choose one",
			COMMAND_HELP_EMBED_DETAILS_FIELD_USAGE_NAME: "Usage",
...
All time (Watchdog): ${watchdog.watchdog_total}`,
			...
			ERROR_MISSING_PERMISSIONS: (
				...
				`To use the command ${alias || command}${
					alias !== command ? ` (${command})` : ""
				}, ${type === "client" ? "I" : "you"} need to have ${
